document.addEventListener('DOMContentLoaded', function() {
    ...
    // Fare hareketine göre 3D efekti - GELİŞTİRİLMİŞ SÜRÜM
    document.addEventListener('mousemove', function(e) {
        const x = e.clientX / window.innerWidth;
        const y = e.clientY / window.innerHeight;
        
        // Tüm eğilebilir kutular için seçicileri genişlet
        const tiltElements = document.querySelectorAll('.profile-card, .tilt-effect, .card, .box');
        
        tiltElements.forEach(element => {
            if (element) {
                element.style.transform = `perspective(1000px) rotateY(${x * 10 - 5}deg) rotateX(${y * -10 + 5}deg)`;
